...
const ARTICLE_SOURCE_PATHS = new Set(['/noticias', '/garagem-dos-sonhos', '/marcas', '/ranking', '/pesquisa']);
...
export default async function NoticiaPage({ params, searchParams }: Props) {
  ...
  const requestedSourcePath = searchParams?.from;
  const fallbackHref = requestedSourcePath && ARTICLE_SOURCE_PATHS.has(requestedSourcePath)
    ? requestedSourcePath
    : '/noticias';
  const articleSection = requestedSourcePath === '/marcas'
    ? 'Marcas'
    : requestedSourcePath === '/garagem-dos-sonhos'
      ? 'Garagem dos Sonhos'
      : requestedSourcePath === '/ranking'
        ? 'Rankings'
        : 'Notícias';
  ...
  return (
    <main className="min-h-screen bg-white">
      <Navbar activeSection={articleSection} />
      <ArticleBackButton fallbackHref={fallbackHref} compact floating />
      ...
      <section className="relative h-[50vh] min-h-[350px] md:h-[60vh] flex items-end overflow-hidden">
        ...
        <div className="relative z-10 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto w-full pb-10">
          <p className="text-[#fca5a5] text-xs font-bold uppercase tracking-[0.28em] font-rajdhani mb-3">
            {articleSection}
          </p>
